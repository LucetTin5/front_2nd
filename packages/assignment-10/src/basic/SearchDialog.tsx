...
const SearchDialog = ({ searchInfo, onClose }: Props) => {
  ...
  const previousSearchOptionsRef = useRef(searchOptions);
  const cachedFilteredLecturesRef = useRef<Lecture[]>([]);

  const filteredLectures = useMemo(() => {
    // 검색 옵션이 변경되지 않았다면 캐시된 결과를 반환
    if (Object.is(previousSearchOptionsRef.current, searchOptions)) {
      return cachedFilteredLecturesRef.current;
    }

    const { query = '', credits, grades, days, times, majors } = searchOptions;
    const newFilteredLectures = lectures
      .filter(
        (lecture) =>
    ...
    cachedFilteredLecturesRef.current = newFilteredLectures;

    return newFilteredLectures;
            ...
            <HStack spacing={4}>
              ...
              <FormControl>
                ...
                >
                  ...
                  <MajorCheckboxList allMajors={allMajors} />
                </CheckboxGroup>
              </FormControl>
            </HStack>
